...
#define Offset 16
#define SegmentSize 6256
#define WINDOW_WIDTH 500
#define WINDOW_HEIGHT 400
const int width = SegmentSize/Offset,height = 1, depth = 1,c = 8;
CImg<short>* result;
CImgDisplay* display;
short* pImgBuff;
CImg<unsigned char> visu(WINDOW_WIDTH, WINDOW_HEIGHT, 1, 3, 0);
CImgDisplay draw_disp(visu, "Intensity profile");
const unsigned char ColChan_1[] = { 255, 0, 0 }, ColChan_2[] = { 0, 255, 0 }, ColChan_3[] = { 0, 0, 255 },ColChan_4[] = { 255, 0, 255 },
					 ColChan_5[] = { 255, 255, 0 }, ColChan_6[] = { 100, 100, 255 }, ColChan_7[] = { 255, 255, 0 },ColChan_8[] = { 255, 255, 255 };
...
int _tmain()
{
	...
	pImgBuff = new short[width* height * c];
	memset(pImgBuff, 0, width*height * depth * sizeof(short));



	result = new CImg<short>(pImgBuff, width, height, 1, 3, true);
	display = new CImgDisplay(*result, "MeeToo");
	display->dumpRender(*result);
	display->dump();
...
void Draw(){//-WINDOW_HEIGHT/c
		//visu.fill(0).draw_graph_spline(result->get_crop(0, 0, 0, 0, result->width() - 1, 0, 0, 0),	-7*WINDOW_HEIGHT/c, ColChan_1, 1, 0, SHRT_MAX*c*2, SHRT_MIN);
		//visu.draw_graph_spline(result->get_crop(0, 0, 0, 1, result->width() - 1, 0, 0, 1),			-6*WINDOW_HEIGHT/c, ColChan_2, 1, 0, SHRT_MAX*c*2, SHRT_MIN);
		//visu.draw_graph_spline(result->get_crop(0, 0, 0, 2, result->width() - 1, 0, 0, 2),			-5*WINDOW_HEIGHT/c, ColChan_3, 1, 0, SHRT_MAX*c*2, SHRT_MIN);
		//visu.draw_graph_spline(result->get_crop(0, 0, 0, 3, result->width() - 1, 0, 0, 3),			-4*WINDOW_HEIGHT/c, ColChan_4, 1, 0, SHRT_MAX*c*2, SHRT_MIN);
		//visu.draw_graph_spline(result->get_crop(0, 0, 0, 4, result->width() - 1, 0, 0, 4),			-3*WINDOW_HEIGHT/c, ColChan_5, 1, 0, SHRT_MAX*c*2, SHRT_MIN);
		//visu.draw_graph_spline(result->get_crop(0, 0, 0, 5, result->width() - 1, 0, 0, 5),			-2*WINDOW_HEIGHT/c, ColChan_6, 1, 0, SHRT_MAX*c*2, SHRT_MIN);
		//visu.draw_graph_spline(result->get_crop(0, 0, 0, 6, result->width() - 1, 0, 0, 6),			-1*WINDOW_HEIGHT/c, ColChan_7, 1, 0, SHRT_MAX*c*2, SHRT_MIN);
		//visu.draw_graph_spline(result->get_crop(0, 0, 0, 7, result->width() - 1, 0, 0, 7),			0,					ColChan_8, 1, 0, SHRT_MAX*c*2, SHRT_MIN).display(draw_disp);

	visu.fill(0).draw_graph_spline(
		result->get_crop(0, 0, 0, 0, result->width() - 1, 0, 0, 0),
		-WINDOW_HEIGHT/c, ColChan_1, 1, 0, SHRT_MAX, SHRT_MIN).display(draw_disp);
	...
	else
	{
		...
		for (int col = 0; col <  width;col++){
			for(int row =0; row < 8; row++){
				pImgBuff[col] = pi16Buffer[col*row*Offset];
			}
		}
	}
